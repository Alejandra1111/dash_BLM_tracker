...
class TestTweetRetweetStats(unittest.TestCase):
    ...
    def test_stat_words(self):
        ...
        # note: stat_words rely on word counts on random samples, so the contents can't be compared without setting seed.
        self.assertTrue(
            all(list(self.stat_words1.columns == expected.columns) + \
                [len(self.stat_words1) == len(expected)]), 'either columns or len do not match')
