...
def city_extended0(city, version):
	result = q.enqueue(city_extended, city, version)
	while result.is_finished == False:
		print('.', sep=' ', end='', flush=True)
		time.sleep(.1)
	
	print('\n',result.get_id(),' is_finished: ',result.is_finished)
...
def update_fig_users_bars(picked_stats, timespan, hour):
	...
	result = q.enqueue(fig_top_users, df, subset=timespan2)
	while result.is_finished == False:
		print('.', sep=' ', end='', flush=True)
		time.sleep(.1)
	
	print('\n',result.get_id(),' is_finished: ',result.is_finished)
	fig = go.Figure(result.result)
	return fig
