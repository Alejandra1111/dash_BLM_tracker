...
def tw_data_files_to_df_json(files, orient='records', lines=False):
    '''append and concat data files into a pandas.DataFrame'''
    df = []
...
def keep_recent_files2(files, base_timestamp, file_type= '.json', days = 14, no_newer=False,
                      prefix = 'BLM_tweet_original_delivery-1-', num_char=11):
    timestamps = [pd.Timestamp(file.split(prefix,1)[1][:num_char]) for file in files ]
    if no_newer: 
        keep_idx1 = [(base_timestamp - timestamp <= pd.Timedelta(days, unit='d')) & 
                     (base_timestamp - timestamp >= pd.Timedelta(0, unit='d')) for timestamp in timestamps]
    else: 
