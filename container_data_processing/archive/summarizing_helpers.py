...
def get_top_tweets(ori_data, cum_rt_data, subset_name=''):
    top15_TW = ori_data.RT_id.value_counts()[:16]
    top15_TW = top15_TW[top15_TW.index!=''][:15]
...
class cumulative_data():
    def __init__(self, cum_ori, cum_rt, cum_words, now=None, hour_stats_only=False):
        self.cum_ori = time_subsets(cum_ori, now=now)
        self.cum_rt = cum_rt
        self.cum_words = time_subsets(cum_words, now=now)
        self.now = now
        self.hour = 'hour_' + str(now.hour) 
        self.hour_stats_only = hour_stats_only

    def add_words_subsets(self):
        subs = self.cum_words

        if self.hour_stats_only:
            stat_words = calc_stat_words(df_words_sample(subs.now_1h))
            stat_words.index = [self.hour]
        else:
            stat_words = (calc_stat_words(df_words_sample(subs.now_1h)).
            ...
            stat_words.index = [self.hour, 'today', 'yesterday', 'seven_days']
    ...
    def add_tweet_subsets(self):
        ...
        if self.hour_stats_only:
            top_tweets = get_top_tweets(subs.now_1h, cum_rt, subset_name=self.hour)
        else:
            top_tweets = (get_top_tweets(subs.now_1h, cum_rt, subset_name=self.hour)
    ...
    def add_user_subsets(self):
        ...
        if self.hour_stats_only:
            top_users = get_top_users(subs.now_1h, cum_rt, subset_name=self.hour)
        else:
            top_users = (get_top_users(subs.now_1h, cum_rt, subset_name=self.hour)
