...
from globals import current_time, current_time_str, data_dest, cities, cities_all, \
    json_split, stat_days_short, stat_days_long
...
def process_city_date_stats():
    ...
    #!! pseudo log_file update for dev
...
def create_or_update_city_date_stats(city, data_path, current_time, process_hours=[], verbose=True):
    ...
    stats = make_stats_json(stat_sentiments, stat_emotions, stat_words, top_tweets, top_users, **json_split)
...
def get_city_date_stats(city, datetime, path, process_hours=[]):
    ...
    data_original = loader.load(dated_files.original, filename_filter_short)
    data_retweet = DataLoaderJson().load_data_from_single_file(db_access, dated_files.retweet.files[0])
    data_words = loader.load(dated_files.words, filename_filter_short)
    data_sentiments = loader.load(dated_files.sentiments, filename_filter_long)
    data_emotions = loader.load(dated_files.emotions, filename_filter_long)
...
def make_stats_json(stat_sentiments, stat_emotions, stat_words, top_tweets, top_users, **kwargs):
    stats = {
